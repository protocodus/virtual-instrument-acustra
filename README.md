...
mic or electrically loaded saddle piezo. Shape and Material directions, steel or nylon
...
the bridge hand, CC68 as legato and, while CC68 is enabled, note-off velocity
...
WAV files are omitted from this repository. Generate the twelve demos locally
...
- **Fretting hand** — under CC68, hammer-ons at
...
| --- | --- | ---: | ---: | ---: |
| `01-steel-sustain-range.wav` | Steel sustain from open E2 to B5, one held pluck at a time | 9.5 s | −15.0 dBFS | +12.0 dB |
| `02-nylon-fingerstyle.wav` | A fingertip nylon arpeggio with overlapping held notes | 4.2 s | −22.2 dBFS | +19.2 dB |
| `03-shape-material-anchors.wav` | One chord: Parlor/Jumbo, then Cedar/Maple anchor settings | 13.7 s | −7.1 dBFS | +4.1 dB |
| `04-string-age.wav` | The same steel phrase with fresh strings, then fully aged strings | 6.5 s | −6.5 dBFS | +3.5 dB |
| `05-alternate-tunings.wav` | Drop D, DADGAD and Open G chords | 12.3 s | −7.8 dBFS | +4.8 dB |
| `06-playing-behaviours.wav` | A chord change over a ringing chord, CC2 bridge-hand damping, then two natural harmonics above the fretted range | 10.7 s | −10.2 dBFS | +7.2 dB |
| `07-fretting-hand.wav` | Hammer-ons at two velocities and a pull-off under CC68, then one fretted note released three ways by its note-off velocity | 8.8 s | −15.7 dBFS | +12.7 dB |
| `08-strummed-chords.wav` | Same-sample chords swept as alternating strums, then one chord eight times hand-damped, no two strokes the same take | 6.9 s | −3.3 dBFS | +0.3 dB |
| `09-recuerdos-de-la-alhambra.wav` | Tarrega, Recuerdos de la Alhambra, bars 1-12: a nylon tremolo over a thumb arpeggio | 31.6 s | −17.9 dBFS | +14.9 dB |
| `10-lagrima.wav` | Tarrega, Lagrima, bars 1-8: a sung nylon melody over held bass | 26.0 s | −20.8 dBFS | +17.8 dB |
| `11-picking-techniques.wav` | Finger, pick and thumb on steel, then on nylon; same notes and velocity | 10.6 s | −15.7 dBFS | +12.7 dB |
| `12-capture-types.wav` | Stereo mic, mono mic, piezo | 7.3 s | −18.6 dBFS | +15.6 dB |
<!-- peaks-table-end -->
...
Ordinary note-off damps the existing note at every release velocity, including
127, without retuning it or adding a new excitation. Release speed becomes an
active finger lift only while the player explicitly enables CC68 legato.
This prevents a fast keyboard key-up from sounding like another stroke.
Disabling CC68 also cancels an active lift waiting under the sustain pedal;
enabling it after an ordinary key-up cannot retroactively turn that release
into a pull-off.

In CC68 mode, MIDI's unsensed/default release velocity of 64 and below keeps
the finger touching the string; above it the finger lifts, to a full pull-off
at 127. The string was pressed to the fret by
...
the whole pressed shape; faster motion then adds no energy. With CC68 enabled,
a keyboard that senses release can leave open strings ringing unless the
player holds the key to the end or damps with CC2. A string lifted to open is nobody's: it rings on in the junction with no key and
...
Piezo includes the electrical high-pass response derived from a measured
...
- the fretting hand: reconstructed ideal-string displacement and velocity
  ...
  note-off. The wrapper maps all ordinary release velocities, including 127,
  to that same damped wave; 127 lifts only with CC68 explicitly enabled.
  A dedicated release suite checks high note-on velocities across both string
  materials, all four body shapes, three registers and 44.1/48/96 kHz, plus
  sustain/CC68 transitions and the immediate released-versus-held peak;
